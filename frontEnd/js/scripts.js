...
function attachSimplifiedTaxEventListeners() {
  ...
  const salesAmountInput = document.querySelector(
    'input[aria-label="매출대가"]'
  );
  const purchaseAmountInput = document.querySelector(
    'input[aria-label="매입대가"]'
  );
  const valueAddedRateInput = document.querySelector(
    'input[aria-label="업종별 부가가치율"]'
  );
  const simplifiedTaxResultInput = document.querySelector(
    'input[aria-label="간이과세 값"]'
  );
  jobButtons.forEach((button) => {
    button.addEventListener("change", () =>
      updateValueAddedRate(button.value, valueAddedRateInput)
    );
  });
  const calculateSimplifiedTax = () => {
    ...
    const valueAddedRate =
      parseFloat(valueAddedRateInput.value.replace("%", "")) / 100;
